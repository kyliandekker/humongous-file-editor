...
namespace resource_editor
{
	...
	namespace chunk_reader
	{
		...
		int32_t jump(ScriptInstruction& args, unsigned char* data, size_t data_size);
		...
		void default_func(unsigned char* data, ArgsAllocator& return_data);

		struct bytecode
		{
			std::string m_Name;
			std::function<void(unsigned char*, ArgsAllocator&)> m_Func = default_func;
		};

		// TODO: Different dictionaries for different version support.
