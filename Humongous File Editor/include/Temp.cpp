	...
	namespace chunk_reader
	{
		...
		void Temp::Read(const char* path)
		{
			...
			while (i < 99999)
			{
				...
				//if (utils::chunkcmp(n.chunk_id, TALK_CHUNK_ID) == 0)
				//{
				//	Node talkChunk = Node(n);
				//	AddEntry(talkChunk.offset, std::to_string(i) + ".wav");
				//}
				//else if (utils::chunkcmp(n.chunk_id, SGEN_CHUNK_ID) == 0)
				//{
				//	Node talkChunk = Node(n);
				//	AddEntry(talkChunk.offset, std::to_string(i) + ".wav");
				//}
