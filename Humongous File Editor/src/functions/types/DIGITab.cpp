...
{
	bool DIGITab::ReplaceResource(chunk_reader::FileContainer*& fc, size_t offset)
	{
		std::string path;
		...
		if (SoundTab::ReplaceResource(path, fmt_chunk, data_chunk))
		{
			...
			fc->Replace(offset, new_data, digi_chunk.ChunkSize());

			free(new_data);
			
			// TODO: HE0 needs to be updated and scripts as well.
