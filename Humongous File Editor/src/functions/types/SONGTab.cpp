...
namespace HumongousFileEditor
{
    bool SONGTab::GetData(chunk_reader::FileContainer*& fc, size_t offset, chunk_reader::SDAT_Chunk& sdat_chunk, chunk_reader::HSHD_Chunk& hshd_chunk)
    {
		...
		size_t hshd_offset = chunk_reader::ChunkFunctions::GetOffsetChunk(fc, sgen_chunk.song_pos, { chunk_reader::HSHD_CHUNK_ID });
		if (hshd_offset == -1)
			return false;
		...
		size_t sdat_offset = chunk_reader::ChunkFunctions::GetOffsetChunk(fc, sgen_chunk.song_pos, { chunk_reader::SDAT_CHUNK_ID });
		if (sdat_offset == -1)
			return false;
